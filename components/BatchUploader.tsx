...
import { Download, LoaderCircle, Play, RotateCcw, Save, UploadCloud, X } from "lucide-react";
...
function createItem(file: File): RecognitionItem {
  return { id: crypto.randomUUID(), file, preview: URL.createObjectURL(file), licensePlate: "", confidence: 0, status: "pending", recordId: null };
}
...
export default function BatchUploader() {
  ...
  const stats = useMemo(() => ({ total: items.length, done: items.filter((item) => item.status === "done").length, error: items.filter((item) => item.status === "error").length, processing: items.filter((item) => item.status === "processing").length }), [items]);
  ...
  function toggleGate(code: GateCode) {
    ...
    if (items.length) setItems((current) => current.map((item) => ({ ...item, status: "pending", error: undefined, recordId: null })));
  }

  async function recognizeOne(item: RecognitionItem) {
    patchItem(item.id, { status: "processing", error: undefined });
    try {
      const form = new FormData(); form.append("file", item.file); gates.forEach((gate) => form.append("gate", gate));
      ...
      patchItem(item.id, { status: "done", licensePlate: plate, confidence: payload.data?.plateConfidence || payload.data?.confidence || 0, recordId: payload.data?.recordId || null, error: payload.data?.storageError || undefined });
    } catch (error) {
      patchItem(item.id, { status: "error", error: error instanceof Error ? error.message : "Có lỗi xảy ra.", recordId: null });
    }
  }

  async function saveManual(item: RecognitionItem) {
    const plate = item.licensePlate.trim().toUpperCase();
    if (!plate || !gates.length || saving[item.id]) return;
    setSaving((current) => ({ ...current, [item.id]: true }));
    try {
      if (item.recordId) {
        const response = await fetch("/api/plate/update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recordId: item.recordId, plate }) });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok || !payload.success) throw new Error(payload.error || "Cập nhật biển số thất bại.");
        patchItem(item.id, { licensePlate: payload.data?.licensePlate || plate, confidence: 1, status: "done", error: undefined });
      } else {
        const form = new FormData(); form.append("file", item.file); form.append("plate", plate); gates.forEach((gate) => form.append("gate", gate));
        const response = await fetch("/api/plate/manual", { method: "POST", body: form });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok || !payload.success) throw new Error(payload.error || "Lưu biển số thủ công thất bại.");
        patchItem(item.id, { licensePlate: payload.data?.licensePlate || plate, confidence: 1, status: "done", recordId: payload.data?.recordId || null, error: undefined });
      }
    } catch (error) {
      patchItem(item.id, { error: error instanceof Error ? error.message : "Không thể lưu biển số." });
    } finally { setSaving((current) => ({ ...current, [item.id]: false })); }
  }

  async function runAll() {
    if (running || resetting || !gates.length) return;
    const queue = items.filter((item) => item.status === "pending" || item.status === "error"); if (!queue.length) return;
  ...
  async function resetDatabase() {
    ...
    if (!window.confirm("Reset toàn bộ dữ liệu test?\n\nThao tác này sẽ xóa các bản ghi trong plate_records và ảnh tương ứng trong Storage. Không thể hoàn tác.")) return;
    setResetting(true);
    try {
    ...
    } catch (error) { window.alert(error instanceof Error ? error.message : "Reset thất bại."); } finally { setResetting(false); }
  }
  ...
  function exportCsv() {
    ...
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `ket-qua-${gates.join("-") || "xe"}-${new Date().toISOString().slice(0, 10)}.csv`; anchor.click(); URL.revokeObjectURL(url);
  }
  ...
  return (
    <main className="shell">
      ...
      <section className="card" style={{ marginBottom: 18 }}><div className="label">Cổng nhận xe</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginTop: 10 }}>
        {GATES.map((item) => { const checked = gates.includes(item.code); return <label key={item.code} style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", border: `1px solid ${checked ? "#111827" : "#e5e7eb"}`, borderRadius: 12, cursor: running ? "not-allowed" : "pointer", background: checked ? "#f3f4f6" : "white" }}><input type="checkbox" checked={checked} disabled={running} onChange={() => toggleGate(item.code)} /><span>{item.name}</span></label>; })}
      </div><p className="hint">Đã chọn: <strong>{gateLabel || "chưa có cổng"}</strong>. Một biển số nhận diện thành công sẽ được cấp quyền tại tất cả các cổng đã chọn.</p></section>
      ...
      <section className={`dropzone ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => gates.length ? inputRef.current?.click() : window.alert("Vui lòng chọn ít nhất một cổng trước.")}>
        <input ref={inputRef} type="file" accept="image/*" multiple hidden onChange={onInput} /><div className="drop-icon"><UploadCloud size={30} /></div><div><h2>Thả nhiều ảnh vào đây</h2><p>{gates.length ? `${gateLabel} · tối đa ${MAX_FILES} ảnh/lần` : "Chọn ít nhất một cổng trước khi chọn ảnh"}</p></div>
      </section>

      {items.length > 0 && <section className="results"><div className="results-head"><div><h2>Kết quả · {gateLabel || "Chưa chọn cổng"}</h2><p>Sửa biển số trực tiếp nếu AI đọc sai hoặc không nhận diện được.</p></div><button className="button secondary" onClick={exportCsv}><Download size={18} /> Xuất CSV</button></div>
        <div className="table-wrap"><table><thead><tr><th>#</th><th>Ảnh</th><th>Cổng</th><th>Biển số</th><th>Độ tin cậy</th><th>Trạng thái</th><th /></tr></thead><tbody>{items.map((item, index) => <tr key={item.id}>
          <td>{index + 1}</td><td><div className="file-cell"><img src={item.preview} alt={item.file.name} /><div><strong>{item.file.name}</strong><span>{(item.file.size / 1024 / 1024).toFixed(2)} MB</span></div></div></td><td>{gateLabel}</td>
          <td><div style={{ display: "flex", gap: 8, alignItems: "center", minWidth: 210 }}><input className="field" value={item.licensePlate} placeholder="Nhập biển số..." onChange={(event) => patchItem(item.id, { licensePlate: event.target.value.toUpperCase() })} /><button className="button secondary" style={{ whiteSpace: "nowrap", padding: "9px 12px" }} onClick={() => saveManual(item)} disabled={!item.licensePlate.trim() || saving[item.id] || item.status === "processing"}>{saving[item.id] ? <LoaderCircle className="spin" size={16} /> : <Save size={16} />}{item.recordId ? "Cập nhật" : "Lưu"}</button></div></td>
          <td>{item.confidence > 0 ? `${(item.confidence * 100).toFixed(1)}%` : "—"}</td><td><Status item={item} /></td><td><button className="icon-button" aria-label="Xóa ảnh" onClick={() => removeItem(item.id)} disabled={item.status === "processing" || saving[item.id]}><X size={17} /></button></td>
