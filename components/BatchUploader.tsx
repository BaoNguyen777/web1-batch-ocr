...
export default function BatchUploader() {
  ...
  async function recognizeOne(item: RecognitionItem) {
    ...
    try {
      ...
      if (!response.ok || (payload.success === false && !payload.data)) throw new Error(payload.error || "Nhận diện thất bại.");
      patchItem(item.id, { status: "done", licensePlate: plate, confidence: payload.data?.confidence || 0, error: undefined });
    } catch (error) {
  ...
  async function runAll() {
    ...
    async function worker() {
      while (cursor < queue.length) await recognizeOne(queue[cursor++]);
    }
    try {
  ...
  async function resetDatabase() {
    ...
    try {
      const response = await fetch("/api/reset", {
        method: "POST",
        headers: { Authorization: `Bearer ${window.prompt("Nhập CRON_SECRET để xác nhận reset:") || ""}` }
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || !payload.success) throw new Error(payload.error || "Reset thất bại.");
      clearAll();
      window.alert(`Reset thành công!\nĐã xóa ${payload.deletedRecords ?? 0} bản ghi và ${payload.deletedImages ?? 0} ảnh.`);
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "Reset thất bại.");
    } finally {
      setResetting(false);
    }
  }
  ...
  function exportCsv() {
    const rows = [["STT", "Tên ảnh", "Biển số", "Độ tin cậy", "Trạng thái"], ...items.map((item, index) => [index + 1, item.file.name, item.licensePlate, item.confidence ? `${(item.confidence * 100).toFixed(2)}%` : "", item.status])];
    ...
    <main className="shell">
      <section className="hero">
        ...
        <div className="hero-actions">
          <button className="button secondary" onClick={clearAll} disabled={!items.length || running || resetting}>
            <RotateCcw size={18} /> Xóa danh sách
          </button>
          <button className="button secondary" onClick={resetDatabase} disabled={running || resetting}>
            {resetting ? <LoaderCircle className="spin" size={18} /> : <RotateCcw size={18} />}
            {resetting ? "Đang reset..." : "Reset dữ liệu"}
          </button>
          <button className="button primary" onClick={runAll} disabled={!items.length || running || resetting}>
      ...
      <section className={`dropzone ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => inputRef.current?.click()}>
        ...
        <div><h2>Thả nhiều ảnh vào đây</h2><p>hoặc bấm để chọn ảnh · tối đa {MAX_FILES} ảnh/lần</p></div>
      </section>

      {items.length > 0 && (
        <section className="results">
          <div className="results-head"><div><h2>Kết quả</h2><p>Chỉ hiển thị biển số khi AI thực sự phát hiện được.</p></div><button className="button secondary" onClick={exportCsv} disabled={!items.length}><Download size={18} /> Xuất CSV</button></div>
          <div className="table-wrap"><table><thead><tr><th>#</th><th>Ảnh</th><th>Biển số</th><th>Độ tin cậy</th><th>Trạng thái</th><th /></tr></thead><tbody>
            {items.map((item, index) => <tr key={item.id}>
              <td>{index + 1}</td>
              <td><div className="file-cell"><img src={item.preview} alt={item.file.name} /><div><strong>{item.file.name}</strong><span>{(item.file.size / 1024 / 1024).toFixed(2)} MB</span></div></div></td>
              <td><input className="field" value={item.licensePlate} placeholder="Không phát hiện" onChange={(event) => patchItem(item.id, { licensePlate: event.target.value.toUpperCase() })} /></td>
              <td>{item.confidence > 0 ? `${(item.confidence * 100).toFixed(1)}%` : "—"}</td>
              <td><Status item={item} /></td>
              <td><button className="icon-button" aria-label="Xóa ảnh" onClick={() => removeItem(item.id)} disabled={item.status === "processing"}><X size={17} /></button></td>
            </tr>)}
          </tbody></table></div>
        </section>
      )}
